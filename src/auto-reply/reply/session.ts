...
function resolveInitSessionStateAttemptContext(
  ...
  const isSystemEvent = isSystemEventProvider(ctx.Provider);
...
async function initSessionStateAttemptLocked(
  ...
  const metaPatch = deriveSessionMetaPatch({
    ...
    groupResolution,
    skipSystemEventMetadata: isSystemEvent,
  });
  if (metaPatch) {
  ...
  if (isNewSession) {
    ...
    sessionEntry.memoryFlushCliPromptTokens = undefined;
    sessionEntry.memoryFlushCliTranscriptBytes = undefined;
    sessionEntry.memoryFlushCliFingerprint = undefined;
