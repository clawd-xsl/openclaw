...
| `session_summaries` | Recall durable summaries of visible prior sessions (memory-core plugin)     |
...
`session_summaries` searches bounded, durable summaries generated by the
memory-core plugin. It applies the same session visibility policy as the other
history tools and is available in the built-in coding and messaging profiles
when memory-core is enabled. The configured summary lookback and literal query
filter keep recall scoped; an explicit tool deny still takes precedence.
