...
Do not expose it publicly. The UI strips dashboard URL tokens from the URL after load
and remembers them in localStorage for the selected gateway URL.
...
- **Token source**: `gateway.auth.token` (or `OPENCLAW_GATEWAY_TOKEN`); `openclaw dashboard` can pass it via URL fragment for one-time bootstrap, and the Control UI remembers it in localStorage for the selected gateway URL after stripping it from the URL.
