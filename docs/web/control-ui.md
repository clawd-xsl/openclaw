...
- `connect.params.auth.password`
  The dashboard settings panel remembers a token in localStorage for the selected gateway URL; passwords are not persisted.
...
- `token` is imported from the URL fragment, stripped from the URL, and remembered in localStorage for the selected gateway URL.
