...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    For transcript reads and writes, import `openclaw/plugin-sdk/session-transcript-runtime` and use `resolveSessionTranscriptIdentity(...)`, `resolveSessionTranscriptTarget(...)`, `readSessionTranscriptEvents(...)`, `readBoundedSessionTranscriptEvents(...)`, `appendSessionTranscriptMessageByIdentity(...)`, `publishSessionTranscriptUpdateByIdentity(...)`, or `withSessionTranscriptWriteLock(...)` with `{ agentId, sessionKey, sessionId }`. These APIs let plugins identify a transcript, read its events, append messages, publish updates, and run related operations under the same transcript write lock. The bounded reader requires explicit byte and event limits, pins one file-size snapshot, reads no more than the requested aggregate byte budget, and returns availability and truncation metadata. Its `unknown` events remain untrusted, so consumers must validate event shapes before using them. Apply `sanitizeSessionTranscriptMessageText(...)` when extracting user text to remove inbound metadata; assistant text is returned unchanged, so callers remain responsible for any domain-specific redaction before crossing another trust boundary. Use the bounded reader for background summarization or recovery paths that must not materialize an entire transcript. Passing `sessionFile`, using `resolveSessionTranscriptLegacyFileTarget(...)`, or importing low-level `appendSessionTranscriptMessage(...)` / `emitSessionTranscriptUpdate(...)` from `openclaw/plugin-sdk/agent-harness-runtime` is deprecated; those paths exist only for legacy code that already receives an active transcript artifact.
