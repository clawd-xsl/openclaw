...
const mocks = vi.hoisted(() => {
  return {
    ...
    connectHook: undefined as ((attempt: number) => Promise<void> | void) | undefined,
    connectCount: 0,
...
vi.mock("@openclaw/signal-ts", () => {
  class MockSignalTsClient {
    ...
    async connect(): Promise<void> {
      ...
      if (mocks.connectHook) {
        await mocks.connectHook(mocks.connectCount);
        return;
      }
...
describe("signal-ts runtime monitor", () => {
  ...
  it("probes durable state and an authenticated connection", async () => {
    const { probeSignalTsAccount } = await import("./signal-ts-runtime.js");
    mocks.connectHook = vi.fn(async () => undefined);

    const result = await probeSignalTsAccount({
      accountInfo: createSignalTsAccountInfo(),
      timeoutMs: 1000,
    });
  ...
  it("reports missing or unusable signal-ts state through the probe", async () => {
    const { probeSignalTsAccount } = await import("./signal-ts-runtime.js");
    ...
    expect(result.error).toContain("state is missing account data");
  ...
  it("retries transient signal-ts content sends", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("retry 2/3"));
  });
  ...
  it("does not expose the monitor client until its connection is ready", async () => {
    ...
    let markFirstConnectStarted: (() => void) | undefined;
    ...
    mocks.connectHook = async (attempt) => {
      if (attempt === 1) {
    ...
    const monitor = monitorSignalTsProvider({
      accountInfo: createSignalTsAccountInfo(),
      abortSignal: abortController.signal,
    ...
    expect(mocks.openRepository).toHaveBeenCalledTimes(2);
    expect(mocks.connectCount).toBe(2);
    releaseFirstConnect?.();
