...
1. **Session summaries and durable memory are retained.** Completed-session
   ...
   has no claim/retry/version/fingerprint model.
2. **Claude CLI remains a first-class product path, but upstream now owns the
   ...
   remaining product gaps are actual block delivery while generation is in
   progress, an explicit isolated tool policy, and CLI-aware durable-memory
   flush.
3. **Direct `signal-ts` transport is retained as a product direction but is
   blocked.** The custom branch depends on `@openclaw/signal-ts` through
   ...
   it. No integration code will be added until the dependency has a legal,
   versioned, installable source.
...
| 5 | `357327b264` | Agents: add session continuity and summaries backend | REDESIGN | Keep summaries and lineage; implement as memory-core plugin state driven by typed lifecycle hooks. |
...
| 8 | `b65ee3a141` | Channels: port Signal and WhatsApp custom reply flows | PARTIAL | Target has structured WhatsApp sticker/reply and Signal quote context. Only missing Signal actions should be rebuilt on the adapter seam. |
...
| 59 | `3a9424bc82` | CLI: use Claude fast mode settings | REDESIGN/CONDITIONAL | Preserve only if production uses `/fast`; add an SDK execution-context field and fingerprint it. |
...
| 79 | `318a8c2ad9` | Signal: support signal-ts stickers | BLOCKED/REDESIGN | Rebuild as an adapter/action capability, not a private runtime branch. |
| 80 | `4e4eb1d2fe` | Signal: finish signal-ts replies and reactions | BLOCKED/REDESIGN | Reuse current structured reply/reaction contracts when the transport exists. |
